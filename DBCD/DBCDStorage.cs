...
namespace DBCD
{
    public class DBCDRow : DynamicObject
    {
        ...
        public object this[string fieldName, int index]
        {
            ...
            set
            {
                var array = (Array)this[fieldName];
                var elementType = array.GetType().GetElementType();
                var convertedValue = value != null && !elementType.IsInstanceOfType(value)
                    ? Convert.ChangeType(value, elementType, System.Globalization.CultureInfo.InvariantCulture)
                    : value;
